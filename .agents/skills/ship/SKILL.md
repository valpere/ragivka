---
name: ship
description: "ragivka ship pipeline: issue → implement → review → merge → close. Usage: /ship [--yes|-y] [issue-id | title]"
---
...
- Numeric or `#N` → `gh issue view {N} --repo valpere/ragivka --json number,title,body,url`
...
```bash
gh issue list --repo valpere/ragivka --assignee @me --state open \
...
```bash
gh issue list --repo valpere/ragivka --state open --search "{title}" \
...
Mark in-progress:
```bash
gh issue edit {ISSUE_NUMBER} --repo valpere/ragivka --add-label "in-progress"
```
...
Agent(Antigravity):
  "Implement: {ISSUE_TITLE}
   ...
   3. Run: golangci-lint run && go test -race ./...
   4. Launch test-generator for new hooks/utils/services — skip if changeset is
      migrations, components, docs, or .agents config files only
...
Mark in-review:
```bash
gh issue edit {ISSUE_NUMBER} --repo valpere/ragivka \
...
```bash
gh pr diff {PR_NUMBER} --repo valpere/ragivka --name-only
```
...
```bash
gh pr merge {PR_NUMBER} --repo valpere/ragivka --squash --delete-branch
```

If checks pending: `gh pr merge {PR_NUMBER} --repo valpere/ragivka --auto --squash --delete-branch`
...
Verify:
```bash
gh pr view {PR_NUMBER} --repo valpere/ragivka --json state,mergedAt --jq '{state,mergedAt}'
```
...
```bash
gh issue comment {ISSUE_NUMBER} --repo valpere/ragivka --body "$(cat <<'EOF'
...
```bash
gh issue close {ISSUE_NUMBER} --repo valpere/ragivka \
