#!/bin/bash
# ragivka project dreaming pass.
#
# Purpose: scheduled (weekly) curation of THIS project only — read-only,
# outputs a report. Scoped to ragivka alone (never cross-mines other
# projects).
#
...
echo "[$(date -Iseconds)] ragivka dreaming pass started" >> "$LOG"
...
EXIT=$?
echo "[$(date -Iseconds)] ragivka dreaming finished (exit=$EXIT, report=$REPORT)" >> "$LOG"
