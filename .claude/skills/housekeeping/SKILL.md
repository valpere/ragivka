---
name: housekeeping
description: "Recurring repo health check. Runs universal hygiene checks and outputs a pass/fail table. Usage: /housekeeping"
---
...
This is the **generic version**. Run `/generate-housekeeping` to create a project-specific
version that adds stack-aware checks (framework version drift, CI coverage delta, lint
pass/fail, migration count, etc.).
...
`CLAUDE.md`). Safe to run on OB Labz and freelance projects with
different layouts.
...
**Fail:** list unformatted files (known baseline: `cmd/server/main.go`, `pkg/db/db.go`, `pkg/obs/cost_test.go` — fix with `gofmt -w <file>`)
