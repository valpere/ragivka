#!/bin/bash
# Stop hook: appends/updates today's entry in .agents/session-log.md.
#
# Behaviour:
...
#
# Output: .agents/session-log.md (gitignored, per-project)
...
if [[ -z "$TRANSCRIPT" || ! -f "$TRANSCRIPT" ]]; then
  ...
  TRANSCRIPT=$(ls -t "$HOME/.agents/projects/$PROJECT_HASH"/*.jsonl 2>/dev/null | head -1 || echo "")
fi
